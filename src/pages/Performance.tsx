...
const Performance: React.FC = () => {
  ...
  const [newObtainedMarks, setNewObtainedMarks] = useState<number | ''>('');
  ...
  const performanceSummary = students.map((student: Student) => {
    const records = performance.filter(record => record.studentId === student.id);
  ...
  const saveNewPerformance = async () => {
    ...
    setNewPercentage(Number(calculatedPercentage.toFixed(2)));
    setSaving(true);
    try {
      ...
      setFormError(err.message || 'Failed to save performance record.');
    }
    setSaving(false);
  };
            ...
            >
              <div className="mb-4">
                ...
                <select
                  ...
                  value={newStudentId !== null ? newStudentId : ''}
                  onChange={(e) => {
                    const val = Number(e.target.value);
                    console.log('Selected student id:', val);
                    setNewStudentId(val);
                  }}
                  required
                >
